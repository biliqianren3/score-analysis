...
def validate_and_clean_data(df):
    ...
    try:
        ...
        # 检查必需列是否存在
        required_columns = ['学号', '姓名', '科目', '分数']
        missing_columns = [col for col in required_columns if col not in cleaned_df.columns]
        
        if missing_columns:
            error_msg = f"上传文件缺少必需列：{', '.join(missing_columns)}。请确保文件包含：学号、姓名、科目、分数。"
            st.error(error_msg)
            return None
        
        # 清理重复值和缺失值
        cleaned_df = cleaned_df.drop_duplicates()
        cleaned_df = cleaned_df.dropna(subset=['学号', '姓名', '科目', '分数'])
        
        # 验证分数范围（建议改为由用户指定满分，或根据科目类型判断）
        if '分数' in cleaned_df.columns:
            cleaned_df['分数'] = pd.to_numeric(cleaned_df['分数'], errors='coerce')
            # 移除分数为NaN的行
            cleaned_df = cleaned_df.dropna(subset=['分数'])
            # 可选：检测分数范围（可增加配置，这里仅作简单警告）
            max_score = cleaned_df['分数'].max()
            if max_score > 150:
                st.warning(f"检测到分数最大值 {max_score}，可能超过正常范围，请核对数据。")
...
with st.sidebar:
            ...
            if uploaded_file:
                        ...
                        with st.form(key='column_mapping_form'):
                            ...
                            if submitted:
                                ...
                                if rename_dict:
                                    ...
                                    if still_missing:
                                        st.error(f"仍然缺少列：{still_missing}，无法处理。请确保所有必需列都已映射。")
                                        st.stop()
                                    else:
                                        st.success("列映射成功！")
                                        # 保存处理后的数据到 session_state
                                        st.session_state.processed_df = df
                                        st.session_state.current_file_name = uploaded_file.name
                                        st.session_state.file_processed = True
                                        st.rerun()  # 重新运行，以便使用处理后的数据
                                else:
                ...
                if processed_df is not None:
                    ...
                    if cleaned_data is not None:
                        ...
                        # 在 session_state 初始化部分确保有 file_metadata
                        ...
                        # 处理完成后，重置文件处理标志，允许下次上传新文件
                        st.session_state.file_processed = False
                        st.session_state.processed_df = pd.DataFrame()
    
        else:
            st.info("⚠️ 请先完成四选二科目的选择（需选择2门），然后才能上传文件。")
            uploaded_file = None  # 确保后续代码不会执行
    ...
    st.sidebar.divider()
    st.sidebar.subheader("🔢 分数类型")
    score_type_global = st.sidebar.radio(
        "分析使用的分数类型",
        ["原始分", "赋分"],
        horizontal=True,
    ...
    if not st.session_state.dashboard_data.empty:
        ...
        # 考试筛选
        exams = ['全部'] + sorted(df['考试名称'].unique().tolist())
        selected_exam = st.selectbox("考试名称", exams, index=0)
        st.session_state.selected_exam = selected_exam
            
        # 科目筛选
        subjects = ['全部'] + sorted(df['科目'].unique().tolist())
        selected_subjects = st.multiselect("科目筛选", subjects, default=['全部'])
        st.session_state.selected_subjects = selected_subjects
            
        # 分数范围筛选
        col1, col2 = st.columns(2)
        with col1:
            min_score = st.slider("最低分", 0, 150, 0)
            st.session_state.min_score = min_score
        with col2:
            max_score = st.slider("最高分", 0, 150, 150)
    ...
    if st.session_state.uploaded_files:
        ...
        if selected_view_file != '全部' and selected_view_file != st.session_state.get('current_view_file'):
            meta = st.session_state.file_metadata.get(selected_view_file)
            if meta:
                st.session_state.physics_or_history = meta['physics_history']
                st.session_state.selected_two = meta['selected_two']
            st.session_state.current_view_file = selected_view_file
            st.rerun()
...
# 模式标识
mode_class = "manual" if st.session_state.manual_mode else "upload"
mode_text = "✍️ 手动输入模式" if st.session_state.manual_mode else "📁 文件上传模式"
...
if not st.session_state.dashboard_data.empty:
    ...
    # 生成宽表
    wide_df = create_student_wide_table(st.session_state.dashboard_data, physics_history, selected_two)
    ...
    if st.button("✅ 确认更新", type="primary", use_container_width=True):
        ...
        if df_input["学号"].isnull().any() or df_input["姓名"].isnull().any():
            ...
            if cleaned_data is not None:
                ...
                st.session_state.data_loaded = True   # 添加此行
                st.success(f"✅ 成功录入 {len(cleaned_data)} 条成绩记录")
                st.rerun()  # 可选：立即重新运行以更新界面    
...
if st.session_state.data_loaded and not st.session_state.dashboard_data.empty:
    ...
    if not st.session_state.filtered_data.empty:
        ...
        current_data = st.session_state.dashboard_data
                ...
                if not comparison_df.empty:
                    ...
                    fig_radar.update_layout(
                        polar=dict(
                            radialaxis=dict(
                                visible=True,
                                range=[0, max(comparison_df['分数_学生'].max(), comparison_df['分数_班级'].max()) * 1.1]
                            )
                        ),
                        height=400,
                        template=st.session_state.chart_config['theme'],
